...
export default function Dashboard() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50/20 text-slate-808 min-h-screen font-sans antialiased text-base">
      ...
      <header className="sticky top-0 z-50 flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-4 py-3 md:px-6 md:py-4 bg-white border-b border-slate-200/80 shadow-sm">
        <div className="flex items-center justify-between w-full md:w-auto">
          <div className="flex items-center gap-3">
            <div className="bg-slate-800 p-2 rounded-xl shadow-sm shrink-0">
              <BarChart2 className="h-5 w-5 text-white" />
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-black tracking-wider uppercase bg-gradient-to-r from-slate-700 to-slate-900 bg-clip-text text-transparent leading-none">
                QuantForge Alpha
              </h1>
              <p className="text-[9px] md:text-xs text-slate-500 font-bold tracking-wider mt-0.5">by Abha Mahato ✦ Options Intelligence Suite</p>
            </div>
          </div>
          {/* Refresh Button on Mobile Header Right */}
          <button 
            onClick={() => { fetchData(activeTab); }}
            disabled={loading}
            title="Refresh data"
            className="md:hidden p-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-all text-slate-600 disabled:opacity-50 active:scale-95"
          >
            <RefreshCw className={`h-4 w-4 transition-transform ${loading ? 'animate-spin text-emerald-500' : 'text-slate-500 hover:text-slate-800'}`} />
          </button>
        </div>

        {/* Global Control Bar */}
        <div className="flex items-center gap-2.5 w-full md:w-auto overflow-x-auto md:overflow-visible scrollbar-none shrink-0">
          {/* Symbol Selector Tab */}
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/80 w-full md:w-auto overflow-x-auto scrollbar-none justify-between md:justify-start">
            {["SENSEX", "BANKEX", "NIFTY", "BANKNIFTY"].map((s) => (
              <button
                key={s}
                onClick={() => setSymbol(s)}
                className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-[11px] md:text-sm font-bold tracking-wider transition-all duration-300 shrink-0 ${
                  symbol === s 
          ...
          <div className="hidden md:flex items-center gap-3 text-xs bg-slate-50 border border-slate-200/80 px-3.5 py-1.5 rounded-xl">
          ...
          <button 
            ...
            disabled={loading}
            title="Refresh data"
            className="hidden md:block p-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-205 transition-all text-slate-600 disabled:opacity-50 active:scale-95"
          >
            <RefreshCw className={`h-4.5 w-4.5 transition-transform ${loading ? 'animate-spin text-emerald-500' : 'text-slate-500 hover:text-slate-808'}`} />
          </button>
        </div>
      </header>
      ...
      {mounted && (
        <div className="bg-[#1e293b] text-slate-100 text-[11px] md:text-xs px-4 py-2.5 md:px-6 flex items-center gap-6 border-b border-[#334155] font-semibold overflow-x-auto whitespace-nowrap scrollbar-none w-full">
          ...
          <div className="md:hidden text-[9px] text-[#64748b] bg-slate-800 px-1.5 py-0.5 rounded font-black uppercase tracking-wider ml-auto shrink-0 animate-pulse">
            Scroll →
          </div>
        </div>
      )}
